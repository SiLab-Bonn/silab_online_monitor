''' This is a producer faking data coming from a Silab readout system
    using std. raw data structure

    This simulation of a data taking works only is the raw data file of
    the readout system contains the following nodes:
      - raw_data node, 32-bit carray
      - meta_data node, pytable with one entry per readout, needed fields
          - index_start, index_stop: start, stop raw data array index for this readout
          - timestamp_start, timestamp_stop: start, stop time stamp for this readout
          - scan_param_id: id of scan parameter
'''
...
from bdaq53 import scan_base


class ReadoutSim(ProducerSim):
