...
m26_dtype = np.dtype([('event_number', '<i8'), ('timestamp', '<u4'), ('trigger_number_begin', '<u2'),
                      ('trigger_number_end', '<u2'), ('plane', '<u2'), ('frame', '<u4'), ('column', '<u2'),
                      ('row', '<u4'), ('trigger_status', '<u2'), ('event_status', '<u2')])


@njit
def correlate_fm(fe_data, m26_data, corr_col, corr_row, dut1, dut2, transpose=True):
    """
    Main function to correlate fe to mimosa data. Correlates data on trigger number, where fe_trigger is assigned to
    a trigger range of mimosa frames. Does consider trigger overflow of FEI4 trigger but does not correlate, skips
    correlation in overflow region (skip approx. 1/32000)
    ...
    dut1: integer; index of active dut 1, needed to determine the dut and correlation order
    dut2: integer; index of active dut 2, needed to determine the dut and correlation order
    transpose: boolean; if True the fe columns/rows correspond to mimosas rows/columns, if False vice versa

    Returns
    -------
    fe_index: int index of m0_data where correlation stops. Data in data buffer below that index will be deleted,
    from will be kept. Next incoming data will be added to data buffer (dict), starting from m0_index
    (sth. like buffer[m0] = buffer[m0][m0_index: ].append(m0_data_new) )

    m26_index: int index of m1_data where correlation stops. Data in data buffer below that index will be deleted,
    from will be kept. Next incoming data will be added to data buffer (dict), starting from m1_index
    (sth. like buffer[m1] = buffer[m1][m1_index: ].append(m1_data_new) )
    """
    ...
    while m26_index < m26_data.shape[0]:  # main loop
        ...
        for m26_i in range(m26_index, m26_data.shape[0]):
            ...
            # skip since we can not correlate; also skip 0 since we do not ant to deal with overflow
            if m26_trigger_begin == 0xFFFF or m26_trigger_end == 0xFFFF or m26_trigger_begin == 0 or m26_trigger_end == 0:
                ...
                if m26_i == m26_data.shape[0] - 1:
                    ...
                    # increase by 1 because in this case we want to delete the last element in buffer as well
            ...
            # keep up fe trigger with mimosa trigger range
            while fe_data[fe_start]['trigger_number'] & 0x7FFF < m26_trigger_begin:
            ...
            # keep up mimosa trigger range with fe trigger if fe trigger is NOT close to overflow
            ...
            # overflow, keep increasing fe_start index and avoid the trigger overflow area
            ...
            while m26_trigger_begin <= fe_data[fe_check]['trigger_number'] & 0x7FFF <= m26_trigger_end:
            ...
            # correlate
            while m26_trigger_begin <= fe_data[fe_index]['trigger_number'] & 0x7FFF <= m26_trigger_end:

                # m26_col corresponds to fe_row and m26_row corresponds to fe_col due to telescope setup
                if transpose:
                ...
                # m26_col corresponds to fe_col and m26_row corresponds to fe_row
                else:
...
def correlate_mm(m0_data, m1_data, corr_col, corr_row):
    """
    Main function to correlate mimosa to mimosa data. Correlates mimosa data on frame number with all permutations
    ...
    Returns
    -------
    m0_index: int index of m0_data where correlation stops. Data in data buffer below that index will be deleted,
    from will be kept. Next incoming data will be added to data buffer (dict), starting from m0_index
    (sth. like buffer[m0] = buffer[m0][m0_index: ].append(m0_data_new) )

    m1_index: int index of m1_data where correlation stops. Data in data buffer below that index will be deleted,
    from will be kept. Next incoming data will be added to data buffer (dict), starting from m1_index
    (sth. like buffer[m1] = buffer[m1][m1_index: ].append(m1_data_new) )
    """
    m0_index = 0
    ...
    # if one mimosa data is empty, return and keep both data in buffer
    if m0_data.shape[0] == 0 or m1_data.shape[0] == 0:
        ...
        for m0_index in range(m0_data.shape[0]):  # go trough first mimosa data m0_data
            ...
            # keep m1_frame up with outer m0_frame
            while m1_index < m1_data.shape[0] - 1 and m1_data[m1_index]['frame'] < m0_frame:
                m1_index += 1
            ...
            # go trough second mimosa data m1_data, start from m1_index which was kept up so you start from same frame
...
def correlate_ff(f0_data, corr_col, corr_row):
    """
    Main function to correlate fei4 data to itself. Takes only one fei4 data input, because in m26 telescope
    we only have one fe reference plane. Just loops over the data and adds to histogramm
    ...
    Returns
    -------
    int which is equal to the length of input data. Since data is equal, everything can be correlated
    """
