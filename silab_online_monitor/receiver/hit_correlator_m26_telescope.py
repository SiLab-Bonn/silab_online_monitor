...
class HitCorrelator(Receiver):
    ...
    def setup_widgets(self, parent, name):
        ...
        for actual_plane in range(1,7):
            ...
            self.tab_widget.addTab(dock_area, 'MIMOSA%d' % actual_plane)
            ...
            for index in range(1,7):
                if actual_plane==index:
                    continue
                else:
                    dock_corr_column=Dock("Cols M26 %d to %d" % (actual_plane , index), size=(50, 50))# % dut_index+1 #, size=(400, 400))
                    dock_corr_row=Dock("Rows M26 %d to %d" % (actual_plane , index), size=(50, 50))  
    ...
    def handle_data(self, data):
        if 'meta_data' not in data:
            for key in data:
                if 'column' == key:
                    for dut_index in range(0,6):
                        self.occupancy_images_columns[dut_index].setImage(data[key][dut_index,:,:], autoDownsample=True)
                if 'row' == key:
                    for dut_index in range(0,6):
                        self.occupancy_images_rows[dut_index].setImage(data[key][dut_index,:,:], autoDownsample=True)
